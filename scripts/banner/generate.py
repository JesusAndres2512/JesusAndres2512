...
ROWS = [
    ("Subject", "Jesus Andres"),
    ("Role", "Cloud Infra Architect · DevSecOps"),
    ("Origin", "Bogota, Colombia"),
    ("Education", "Ing. de Sistemas · Los Libertadores"),
    ("Status", "Securing + Automating + Shipping"),
    ("ToolChain", "VS Code · Git · Linux"),
    ("Core.Cloud", "Azure · AWS · Docker"),
    ("Core.IaC", "Terraform · Terragrunt"),
    ("Core.CI/CD", "GitHub Actions · Jenkins · SonarCloud"),
    ("Core.Backend", "Java/Spring · Python/FastAPI · Node"),
    ("Core.Security", "IAM · JWT · API Gateway"),
    ("Grid.Mail", "andrescabezase@outlook.com"),
    ("Grid.LinkedIn", "/in/jesus-a-cabezas-e-"),
    ("Grid.GitHub", "JesusAndres2512"),
    ("Grid.Phone", "+57 314 229 8292"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # Cloud silhouette: overlapping lobes on a flat base, Azure/AWS-flavoured.
    cloud = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(cloud)
    base_top, base_bottom = 235, 300
    d.rounded_rectangle((70, base_top, 330, base_bottom), radius=32, fill="black")
    d.ellipse((60, 175, 190, 305), fill="black")
    d.ellipse((130, 110, 290, 270), fill="black")
    d.ellipse((215, 165, 345, 295), fill="black")
    d.ellipse((155, 190, 255, 290), fill="black")
    logos["cloud"] = cloud
    ...
    # Security shield with a checkmark cutout.
    shield = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(shield)
    outline = [
        (200, 40), (320, 85), (320, 190),
        (320, 290), (200, 365), (80, 290),
        (80, 190), (80, 85),
    ]
    d.polygon(outline, fill="black")
    check = [(140, 205), (185, 250), (270, 150), (250, 132), (185, 210), (160, 185)]
    d.polygon(check, fill=(0, 0, 0, 0))
    d.line([(140, 205), (182, 248)], fill=(0, 0, 0, 0), width=1)
    mask = Image.new("L", (size, size), 0)
    md = ImageDraw.Draw(mask)
    md.line([(135, 205), (180, 250)], fill=255, width=26, joint="curve")
    md.line([(180, 250), (275, 140)], fill=255, width=26, joint="curve")
    shield_arr = np.array(shield.getchannel("A"))
    mask_arr = np.array(mask)
    shield_arr[mask_arr > 0] = 0
    shield.putalpha(Image.fromarray(shield_arr))
    logos["shield"] = shield
...
def render_svg(
    ...
    cloud = transport(source, logo_points["cloud"][:n])
    code = transport(cloud, logo_points["code"][:n])
    ...
    frames = [source, source, cloud, cloud, code, code, shield, shield, source]
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Jesus Andres' live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "cloud, code, and shield silhouettes.</desc>",
        "<defs>",
    ...
    for band in range(94):
        ...
        delta = (cloud_centroid - centroid) * 0.18 + noise[band]
        d = point_path(pts)
        parts.append(
    ...
    parts.extend(
        [
            ...
            'font-weight="700">@JesusAndres2512</text>',
        ]
    )
